...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('organization_id', 'name', name='uq_dimension_set_org_name')
    )
    op.create_index(op.f('ix_dimension_sets_organization_id'), 'dimension_sets', ['organization_id'], unique=False)
    ...
    op.create_index(op.f('ix_highlight_type_registries_organization_id'), 'highlight_type_registries', ['organization_id'], unique=True)
    ...
    op.create_foreign_key('fk_streams_dimension_set', 'streams', 'dimension_sets', ['dimension_set_id'], ['id'])
    op.create_foreign_key('fk_streams_type_registry', 'streams', 'highlight_type_registries', ['type_registry_id'], ['id'])
    ...
    op.create_foreign_key('fk_batches_dimension_set', 'batches', 'dimension_sets', ['dimension_set_id'], ['id'])
    op.create_foreign_key('fk_batches_type_registry', 'batches', 'highlight_type_registries', ['type_registry_id'], ['id'])
    
    # Remove old highlight_type column and add new highlight_types JSON column
    op.drop_column('highlights', 'highlight_type')
    op.add_column('highlights', sa.Column('highlight_types', sa.JSON(), nullable=False, server_default='[]'))
...
def downgrade() -> None:
    ...
    # Drop triggers
    op.execute('DROP TRIGGER IF EXISTS update_dimension_sets_updated_at ON dimension_sets')
    op.execute('DROP TRIGGER IF EXISTS update_highlight_type_registries_updated_at ON highlight_type_registries')
    
    # Restore old highlight_type column and remove new one
    op.add_column('highlights', sa.Column('highlight_type', sa.String(50), nullable=False, server_default='custom'))
    ...
    # Drop tables
    op.drop_index(op.f('ix_highlight_type_registries_organization_id'), table_name='highlight_type_registries')
    op.drop_table('highlight_type_registries')
    
    op.drop_index(op.f('ix_dimension_sets_organization_id'), table_name='dimension_sets')
