...
class RTMPHandshake:
    ...
    def generate_c1(cls) -> bytes:
        ...
        random_data = bytes([random.randint(0, 255) for _ in range(cls.HANDSHAKE_SIZE - 8)])
...
class RTMPChunkStream:
    ...
    def parse_chunk_header(self, data: bytes, offset: int = 0) -> Tuple[RTMPChunkHeader, int]:
        ...
        if (format_type in [RTMPChunkType.FORMAT_0, RTMPChunkType.FORMAT_1, RTMPChunkType.FORMAT_2] and 
            (header.timestamp >= 0xFFFFFF or header.timestamp_delta >= 0xFFFFFF)):
            
            if len(data) < offset + bytes_consumed + 4:
                raise ValueError("Insufficient data for extended timestamp")
            
            extended_timestamp = struct.unpack('>I', data[offset + bytes_consumed:offset + bytes_consumed + 4])[0]
    ...
    def create_chunk_header(self, header: RTMPChunkHeader) -> bytes:
        ...
        if header.format_type == RTMPChunkType.FORMAT_0:
            ...
            timestamp_delta = header.timestamp_delta if header.timestamp_delta < 0xFFFFFF else 0xFFFFFF
...
class AMFEncoder:
    ...
    def encode_string(cls, value: str) -> bytes:
        ...
        if len(utf8_bytes) > 65535:
            ...
            return bytes([cls.AMF0_LONG_STRING]) + struct.pack('>I', len(utf8_bytes)) + utf8_bytes
        else:
            return bytes([cls.AMF0_STRING]) + struct.pack('>H', len(utf8_bytes)) + utf8_bytes
    ...
    def decode_object(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            if name_length == 0:
                ...
                if self.offset + 1 > len(self.data) or self.data[self.offset] != AMFEncoder.AMF0_OBJECT_END:
...
class RTMPProtocol:
    ...
    def create_connect_message(self, app_name: str, connection_params: Optional[Dict[str, Any]] = None) -> bytes:
    ...
    def create_message_chunks(self, message: RTMPMessage) -> bytes:
        ...
        while offset < len(data):
            if offset == 0:
                ...
                continuation_header_bytes = self.chunk_stream.create_chunk_header(continuation_header)
                chunk_data = data[offset:offset + chunk_size]
    ...
    async def send_connect_command(self, writer: asyncio.StreamWriter, app_name: str, 
                                 connection_params: Optional[Dict[str, Any]] = None) -> None:
    ...
    def create_set_peer_bandwidth_message(self, window_size: int, limit_type: int = 2) -> bytes:
    ...
    async def parse_message(self, reader: asyncio.StreamReader) -> Optional[RTMPMessage]:
        ...
        try:
            ...
            if header.format_type == RTMPChunkType.FORMAT_0:
                ...
                prev_header = self.chunk_stream.last_chunk_headers.get(header.chunk_stream_id)
                if not prev_header:
                    logger.warning(f"No previous header for chunk stream {header.chunk_stream_id}")
                    return None
            ...
            while remaining > 0:
                ...
                if not chunk_data:
                    logger.warning("Unexpected end of stream while reading message data")
                    return None
                ...
                if remaining > 0:
                    ...
                    expected_byte = (RTMPChunkType.FORMAT_3.value << 6) | (header.chunk_stream_id & 0x3F)
                    if cont_header_data[0] != expected_byte:
                        logger.warning(f"Unexpected continuation chunk header: {cont_header_data[0]:02x}")
