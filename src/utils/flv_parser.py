...
class FLVParser:
    ...
    def parse_header(self) -> FLVHeader:
        ...
        try:
            ...
            logger.debug(f"Parsed FLV header: version={version}, audio={has_audio}, video={has_video}")
            return self.header
        ...
        try:
            ...
            if sound_format == FLVSoundFormat.AAC and remaining_size > 0:
                ...
                if aac_packet_type == 0:  # AAC sequence header
                    ...
                    logger.debug(f"Stored AAC sequence header: {len(sequence_data)} bytes")
    ...
    def parse_video_tag(self, data_size: int) -> FLVVideoTag:
        ...
        try:
            if data_size == 0:
                return FLVVideoTag(
                    frame_type=FLVVideoFrameType.KEY_FRAME,
                    codec_id=FLVVideoCodec.AVC
                )
            ...
            if codec_id == FLVVideoCodec.AVC and remaining_size >= 4:
                ...
                if avc_packet_type == FLVAVCPacketType.SEQUENCE_HEADER:
                    ...
                    logger.debug(f"Stored AVC sequence header: {len(sequence_data)} bytes")
    ...
    def extract_video_frames(self, tags: List[FLVTag]) -> List[Tuple[int, bytes, bool]]:
        ...
        for tag in tags:
            if (isinstance(tag.data, FLVVideoTag) and 
                tag.data.codec_id == FLVVideoCodec.AVC and
                tag.data.avc_packet_type == FLVAVCPacketType.NALU):
    ...
    def extract_audio_samples(self, tags: List[FLVTag]) -> List[Tuple[int, bytes]]:
        ...
        for tag in tags:
            if (isinstance(tag.data, FLVAudioTag) and 
                tag.data.sound_format == FLVSoundFormat.AAC and
                tag.data.aac_packet_type == 1):  # AAC raw data
        ...
        try:
            ...
            if not self.header_parsed:
                try:
                    ...
                    logger.info(f"FLV stream: audio={header.has_audio}, video={header.has_video}")
                except Exception as e:
            ...
            if len(self.buffer) > self.buffer_size:
                logger.warning(f"FLV buffer overflow, discarding {len(self.buffer) - self.buffer_size} bytes")
