        ...
        try:
            ...
            if len(request.items) > max_batch_size:
                return BatchCreateResult(
                    ...
                    errors=[f"Batch size {len(request.items)} exceeds limit of {max_batch_size}"]
                )
                ...
                result.items = [
                    {
                        ...
                        "processed_at": item.processed_at.iso_string if item.processed_at else None
                    }
                    for item in items
                ]
        ...
        try:
            ...
            if request.item_ids:
                ...
                for item_id in request.item_ids:
                    ...
                    if item and item.batch_id == batch.id and item.status == BatchItemStatus.FAILED:
                        items.append(item)
            else:
            ...
            for item in items:
                ...
                item.retry_count = item.retry_count + 1 if hasattr(item, 'retry_count') else 1
