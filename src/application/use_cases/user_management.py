...
class UserManagementUseCase(UseCase[UpdateProfileRequest, UpdateProfileResult]):
    ...
    async def update_profile(self, request: UpdateProfileRequest) -> UpdateProfileResult:
        """Update user profile.
    ...
    async def change_password(self, request: ChangePasswordRequest) -> ChangePasswordResult:
    ...
    async def get_profile(self, request: GetProfileRequest) -> GetProfileResult:
        ...
        try:
            ...
            return GetProfileResult(
                status=ResultStatus.FAILURE,
                errors=[f"Failed to get profile: {str(e)}"]
            )
