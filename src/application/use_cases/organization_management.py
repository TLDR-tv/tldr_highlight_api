...
class OrganizationManagementUseCase(
    UseCase[GetOrganizationRequest, GetOrganizationResult]
):
    ...
    async def get_organization(
        self, request: GetOrganizationRequest
        ...
        try:
            ...
            if not org:
                return GetOrganizationResult(
                    status=ResultStatus.NOT_FOUND, errors=["Organization not found"]
                )
            ...
            usage_stats = await self.org_service.get_current_usage(
    ...
    async def update_organization(
        self, request: UpdateOrganizationRequest
    ...
    async def add_member(self, request: AddMemberRequest) -> AddMemberResult:
        ...
        try:
            ...
            return AddMemberResult(status=ResultStatus.NOT_FOUND, errors=[str(e)])
        except UnauthorizedAccessError as e:
            return AddMemberResult(status=ResultStatus.UNAUTHORIZED, errors=[str(e)])
            ...
            return AddMemberResult(
                status=ResultStatus.FAILURE, errors=[f"Failed to add member: {str(e)}"]
            )
    ...
    async def remove_member(self, request: RemoveMemberRequest) -> RemoveMemberResult:
        ...
        try:
            ...
            return RemoveMemberResult(
                status=ResultStatus.SUCCESS, message="Member removed successfully"
            )

        except EntityNotFoundError as e:
            return RemoveMemberResult(status=ResultStatus.NOT_FOUND, errors=[str(e)])
        except UnauthorizedAccessError as e:
            return RemoveMemberResult(status=ResultStatus.UNAUTHORIZED, errors=[str(e)])
        except BusinessRuleViolation as e:
            return RemoveMemberResult(
                status=ResultStatus.VALIDATION_ERROR, errors=[str(e)]
            )
    ...
    async def list_members(self, request: ListMembersRequest) -> ListMembersResult:
        ...
        try:
            # Check permission
            if not await self.org_service.user_has_access(
    ...
    async def get_usage_stats(
        self, request: GetUsageStatsRequest
    ) -> GetUsageStatsResult:
        ...
        try:
            ...
            current_usage = await self.org_service.get_current_usage(
                request.organization_id
            )
    ...
    async def upgrade_plan(self, request: UpgradePlanRequest) -> UpgradePlanResult:
        ...
        try:
            ...
            return UpgradePlanResult(status=ResultStatus.NOT_FOUND, errors=[str(e)])
        except UnauthorizedAccessError as e:
            return UpgradePlanResult(status=ResultStatus.UNAUTHORIZED, errors=[str(e)])
        except BusinessRuleViolation as e:
            return UpgradePlanResult(
                status=ResultStatus.VALIDATION_ERROR, errors=[str(e)]
            )
