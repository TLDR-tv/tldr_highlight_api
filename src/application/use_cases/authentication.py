        ...
        try:
            ...
            if request.organization_name:
                org = await self.org_service.create_organization(
                    owner_id=saved_user.id, name=request.organization_name
                )
            ...
            return RegisterResult(status=ResultStatus.VALIDATION_ERROR, errors=[str(e)])
        except Exception as e:
            return RegisterResult(
                status=ResultStatus.FAILURE, errors=[f"Registration failed: {str(e)}"]
            )
        ...
        try:
            ...
            if request.create_api_key:
                key_name = (
                    ...
                    or f"Session key {datetime.utcnow().isoformat()}"
                )
                ...
                expires_at = (
                    api_key_entity.expires_at.value
                    if api_key_entity.expires_at
                    else None
                )
            ...
            return LoginResult(
                status=ResultStatus.FAILURE, errors=[f"Login failed: {str(e)}"]
            )
        ...
        try:
            ...
            if not api_key:
                return ValidateAPIKeyResult(
                    status=ResultStatus.UNAUTHORIZED, errors=["Invalid API key"]
                )
            ...
            if request.required_scopes:
                ...
                if missing_scopes:
                    return ValidateAPIKeyResult(
                        status=ResultStatus.UNAUTHORIZED,
                        errors=[
                            f"Missing required scopes: {', '.join(s.value for s in missing_scopes)}"
                        ],
                    )
            ...
            return ValidateAPIKeyResult(
                status=ResultStatus.FAILURE, errors=[f"Validation failed: {str(e)}"]
            )
        ...
        try:
            ...
            if not api_key:
                return RevokeAPIKeyResult(
                    status=ResultStatus.NOT_FOUND, errors=["API key not found"]
                )
            ...
            return RevokeAPIKeyResult(
                status=ResultStatus.SUCCESS, message="API key revoked successfully"
            )
