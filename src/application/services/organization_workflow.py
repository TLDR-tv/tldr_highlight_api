...
class OrganizationWorkflow:
    ...
    async def create_organization(
        ...
        if existing:
            raise DuplicateEntityError(entity_type="Organization", existing_value=name)

        # Create organization using factory method
        org = Organization.create(
            name=company_name, owner_id=owner_id, plan_type=plan_type
        )
    ...
    async def deactivate_organization(
        ...
        self.logger.info(
            "Organization deactivated", organization_id=organization_id, reason=reason
        )
