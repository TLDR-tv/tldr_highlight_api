...
class DimensionSetService:
    ...
    async def get_dimension_set_for_stream(
        ...
        if dimension_set_id:
            ...
            dimension_set = await self.dimension_set_repository.get_by_id(
                dimension_set_id
            )
            if not dimension_set:
                raise EntityNotFoundError(
                    entity_type="DimensionSet", entity_id=dimension_set_id
                )
        ...
        org_sets = await self.dimension_set_repository.get_by_organization(
            organization_id=organization_id, active_only=True
        )
    ...
    async def get_default_dimension_set(
        ...
        return await self.get_dimension_set_for_stream(
            dimension_set_id=None, organization_id=organization_id, user_id=user_id
        )
