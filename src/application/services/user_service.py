...
class UserService:
    ...
    async def refresh_tokens(
        ...
        if not user or not user.is_active:
            logger.warning("Token refresh failed - user not found or inactive", user_id=str(user_id))
            return None, None
    ...
    async def update_user_role(
        ...
        if user.role == UserRole.ADMIN and role != UserRole.ADMIN:
            admin_count = len([
                u for u in await self.user_repository.list_by_organization(user.organization_id)
