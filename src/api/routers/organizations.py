...
from src.api.dependencies.auth import require_scopes, get_current_user
...
async def get_organization(
    ...
    use_case: OrganizationManagementUseCase = Depends(get_organization_management_use_case),
):
...
async def update_organization(
    ...
    if result.status == ResultStatus.NOT_FOUND:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.errors[0] if result.errors else "Failed to update organization"
        )
...
async def list_organization_users(
    ...
    if result.status == ResultStatus.NOT_FOUND:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.errors[0] if result.errors else "Failed to list organization users"
        )
...
async def add_user_to_organization(
    ...
    if result.status == ResultStatus.NOT_FOUND:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=result.errors[0] if result.errors else "Organization or user not found"
        )
    elif result.status == ResultStatus.UNAUTHORIZED:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.errors[0] if result.errors else "Failed to add user to organization"
        )
...
async def remove_user_from_organization(
    ...
    if result.status == ResultStatus.NOT_FOUND:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=result.errors[0] if result.errors else "Cannot remove organization owner"
        )
    elif not result.is_success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.errors[0] if result.errors else "Failed to remove user from organization"
        )
...
async def get_organization_usage(
    ...
    if org_result.status == ResultStatus.NOT_FOUND:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=org_result.errors[0] if org_result.errors else "Failed to get organization"
        )
    ...
    if not usage_result.is_success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=usage_result.errors[0] if usage_result.errors else "Failed to get usage statistics"
        )
