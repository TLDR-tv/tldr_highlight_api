...
class OrganizationMapper:
    ...
    @staticmethod
    def to_get_organization_request(organization_id: int, user_id: int) -> GetOrganizationRequest:
    ...
    def to_update_organization_request(
        ...
        return UpdateOrganizationRequest(
            ...
            settings={"plan_type": update_dto.plan_type.value} if update_dto.plan_type else None
        )
    ...
    def to_add_member_request(
        organization_id: int,
        user_id: int,
        add_user_dto: AddUserToOrganizationRequest
    ) -> AddMemberRequest:
    ...
    def to_organization_users_list_response(
        ...
        user_responses = [
            OrganizationMapper.to_organization_user_response(user)
            for user in users
        ]
    ...
    def to_organization_usage_stats(
        ...
        if plan_limits.get("monthly_batch_videos", 0) > 0:
            usage_percentage["batch_videos"] = (
                current_usage["batch_videos_processed"] / plan_limits["monthly_batch_videos"]
            ) * 100
