    ...
    async def validate_api_key(
        ...
        try:
            ...
            if cached_result:
                ...
                api_key_obj = await self._get_api_key_by_id(cached_result["id"], db)
            ...
            return APIKeyValidationResult(
                is_valid=False, error_message="Invalid or expired API key"
            )

        except Exception as e:
            logger.error(f"Error validating API key: {e}")
            return APIKeyValidationResult(
                is_valid=False, error_message="API key validation error"
            )

    async def _get_api_key_by_id(
        self, api_key_id: int, db: AsyncSession
    ) -> Optional[APIKey]:
        """Get API key by ID with user and organization data."""
        stmt = (
            select(APIKey)
            .options(selectinload(APIKey.user).selectinload(User.owned_organizations))
