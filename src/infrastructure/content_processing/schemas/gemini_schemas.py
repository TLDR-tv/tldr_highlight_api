    ...
    def validate_scores(cls, v):
        ...
        for dim_id, score in v.items():
            if not 0.0 <= score <= 1.0:
                raise ValueError(f"Score for dimension '{dim_id}' must be between 0.0 and 1.0, got {score}")
        return v
...
class SceneDescription(BaseModel):
    ...
    description: str = Field(
        description="What's visible or happening in the scene"
    )
    objects_detected: Optional[List[str]] = Field(
        default=None,
        description="Objects or entities detected in the scene"
    )
    actions_detected: Optional[List[str]] = Field(
        default=None,
        description="Actions or movements detected"
    )
...
class GeminiHighlight(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0,
        le=1.0,
        description="Confidence score for this highlight"
    )
    type: str = Field(
        default="general",
    ...
    ranking_score: float = Field(
        ge=0.0,
        le=1.0,
        description="Overall ranking score based on all dimensions"
    )
    ...
    transcript_excerpt: Optional[str] = Field(
        default=None,
        description="Relevant transcript from this highlight"
    )
    viewer_impact: Optional[str] = Field(
        default=None,
        description="Expected viewer reaction or impact"
    )
...
class SegmentQuality(BaseModel):
    ...
    overall_score: float = Field(
        ge=0.0,
        le=1.0,
        description="Overall quality score of the segment"
    )
    ...
    content_richness: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="How content-rich this segment is"
    )
    engagement_potential: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
        description="Potential for viewer engagement"
    )
...
class GeminiVideoAnalysis(BaseModel):
    ...
    highlights: List[GeminiHighlight] = Field(
        description="List of detected highlights"
    )
    transcript: Optional[str] = Field(
        default=None,
        description="Full transcript of speech and important audio"
    )
    scene_descriptions: Optional[List[SceneDescription]] = Field(
        default=None,
        description="Descriptions of key visual moments"
    )
    ...
    content_summary: Optional[str] = Field(
        default=None,
        description="Brief summary of the segment's content"
    )
    recommended_clips: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Suggested clip boundaries for optimal highlights"
    )
...
class HighlightRefinement(BaseModel):
    ...
    highlight_id: str = Field(
        description="ID of the highlight being refined"
    )
    quality_score: float = Field(
        ge=0.0,
        le=1.0,
        description="Quality assessment of the highlight"
    )
    adjusted_start_time: Optional[str] = Field(
        default=None,
        description="Suggested adjusted start time"
    )
    adjusted_end_time: Optional[str] = Field(
        default=None,
        description="Suggested adjusted end time"
    )
    improvement_suggestions: List[str] = Field(
        default_factory=list,
        description="Suggestions for improving the highlight"
    )
    merge_with: Optional[str] = Field(
        default=None,
        description="ID of another highlight to merge with"
    )
    should_keep: bool = Field(
        default=True,
        description="Whether this highlight should be kept"
    )
...
class HighlightRefinementBatch(BaseModel):
    ...
    overall_quality: float = Field(
        ge=0.0,
        le=1.0,
        description="Overall quality of the highlight batch"
    )
    optimization_notes: Optional[str] = Field(
        default=None,
        description="General notes about optimization opportunities"
    )