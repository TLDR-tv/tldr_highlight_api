            ...
            for attempt in range(self.max_retries):
                try:
                    response = await self._call_gemini_structured(
                        content_parts,
                        response_schema=GeminiDimensionEvaluation
                    )
                    ...
                    if self.validate_evaluation_result(result, dimension):
                        ...
                        span.set_attribute("evaluation.confidence", result.confidence.value)
                        return result
...
"""
    ...
    async def evaluate_all_dimensions_with_context(
        ...
        for dim_id, dimension in context.dimension_set.dimensions.items():
            if dimension.dimension_type in ["semantic", "numeric"] and \
               dimension.category in ["emotional", "contextual", "entertainment"]:
