...
class OrganizationModel(Base):
    ...
    streams = relationship("StreamModel", back_populates="organization", cascade="all, delete-orphan")
    api_keys = relationship("APIKeyModel", back_populates="organization", cascade="all, delete-orphan")
    wake_word_configs = relationship("WakeWordModel", back_populates="organization", cascade="all, delete-orphan")
...
class StreamModel(Base):
    ...
    source_type = Column(SQLEnum(StreamSource), default=StreamSource.DIRECT_URL, nullable=False)
    status = Column(SQLEnum(StreamStatus), default=StreamStatus.PENDING, nullable=False, index=True)
    ...
    highlights = relationship("HighlightModel", back_populates="stream", cascade="all, delete-orphan")
...
class UsageRecordModel(Base):
    ...
    # What was used
    resource_type = Column(String(50), nullable=False)  # "stream", "highlight", "api_call"
