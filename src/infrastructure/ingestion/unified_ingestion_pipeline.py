...
from ..streaming.segment_processor import SegmentProcessor, SegmentConfig, ProcessedSegment
...
class StreamIngestionPipeline:
    ...
    async def process_stream(self) -> AsyncIterator[IngestionResult]:
        ...
        try:
            ...
            async for segment in self.segment_processor.process_stream(
                stream_id=self.config.stream_url,
    ...
    def _update_metrics(self, processing_time: float) -> None:
        ...
        if self._metrics["segments_processed"] > 0:
            self._metrics["avg_processing_time"] = (
                self._metrics["total_processing_time"] / self._metrics["segments_processed"]
            )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        health = {
            ...
            "stream_connected": self.stream_adapter.is_connected if self.stream_adapter else False,
            "stream_healthy": self.stream_adapter.is_healthy if self.stream_adapter else False,
            "buffer_health": "healthy",  # Would check buffer status
            "processing_health": "healthy" if self.status == IngestionStatus.PROCESSING else "unhealthy",
            "error_rate": self._metrics["errors"] / max(self._metrics["segments_processed"], 1),
        }
