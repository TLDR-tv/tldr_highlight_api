...
class CircularVideoBuffer:
    ...
    def __init__(self, config: Optional[BufferConfig] = None):
        ...
        self._frame_pool: List[VideoFrame] = (
            [] if self.config.enable_memory_pooling else None
        )
        ...
        logger.info(
            f"Initialized CircularVideoBuffer with max memory: {self.config.max_memory_mb}MB"
        )
    ...
    async def create_window(
        self, start_time: float, duration: float, overlap: float = 0.0
        ...
        while True:
            try:
                ...
                with self._lock if self._lock else nullcontext():
                    ...
                    if self._weak_refs:
                        self._weak_refs = [
                            ref for ref in self._weak_refs if ref() is not None
                        ]
            ...
            try:
                ...
                with self._lock if self._lock else nullcontext():
                    ...
                    if self._frame_count > 0:
                        ...
                        self._stats["keyframe_ratio"] = keyframe_count / len(
                            self._frames
                        )
        ...
        stats.update(
            {
                ...
                "oldest_frame_timestamp": self._frames[0].timestamp
                if self._frames
                else None,
                "newest_frame_timestamp": self._frames[-1].timestamp
                if self._frames
                else None,
...
class VideoBufferManager:
    ...
    def get_buffer(
        self, stream_id: str, config: Optional[BufferConfig] = None
    ) -> CircularVideoBuffer:
