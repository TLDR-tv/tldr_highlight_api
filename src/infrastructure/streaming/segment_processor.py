...
from .video_buffer import VideoFrame, CircularVideoBuffer
...
class ComplexityAnalyzer:
    ...
    def add_frame_metrics(
        self, motion_score: float, scene_change_score: float, audio_level: float
    ) -> None:
    ...
    def calculate_complexity(self) -> float:
        ...
        # Weighted average
        complexity = (
            0.4 * motion_complexity + 0.4 * scene_complexity + 0.2 * audio_complexity
        )
    ...
    def predict_segment_boundary(self) -> bool:
        ...
        recent_motion = (
            list(self.motion_history)[-5:] if len(self.motion_history) >= 5 else []
        )
...
class SegmentProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized SegmentProcessor with strategy: {self.config.strategy}"
        )
    ...
    async def _create_fixed_segments(
        ...
        while True:
            ...
            segment = await self._create_segment(
                frames, current_time, segment_end, SegmentStrategy.FIXED_DURATION
            )
    ...
    async def _create_sliding_segments(
        ...
        while True:
            ...
            overlap_ratio = (
                self.config.overlap_seconds / self.config.segment_duration_seconds
            )
    ...
    async def _create_keyframe_segments(
        ...
        while True:
            ...
            if (
                keyframe_found
                and len(frames_buffer) >= self.config.min_frames_per_segment
            ):
                should_segment = True
                segments_without_keyframe = 0
    ...
    async def _create_scene_segments(
        ...
        logger.warning(
            "Scene-based segmentation not fully implemented, falling back to adaptive"
        )
    ...
    async def _create_adaptive_segments(
        ...
        while True:
            ...
            if (
                ...
                # Create segment
                segment = await self._create_segment(
                    frames_buffer, segment_start, current_time, SegmentStrategy.ADAPTIVE
                )
    ...
    async def _create_event_segments(
        ...
        logger.warning(
            "Event-driven segmentation not implemented, falling back to fixed duration"
        )
    ...
    async def _create_segment(
        ...
        keyframe_indices = [i for i, frame in enumerate(frames) if frame.is_keyframe]
    ...
    def _update_stats(self, segment: ProcessedSegment) -> None:
        ...
        self._stats["avg_segment_duration"] = (
            alpha * segment.duration + (1 - alpha) * self._stats["avg_segment_duration"]
        )
