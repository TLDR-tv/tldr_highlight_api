...
from .video_buffer import VideoFrame, FrameType, BufferFormat
...
class SyncConfig:
    ...
    # Buffer sizes
    timestamp_history_size: int = 1000    # Number of timestamps to keep for drift calculation
...
class FrameSynchronizer:
    ...
    def _create_mapping(
        ...
        if source == TimestampFormat.EPOCH_MILLIS and target == TimestampFormat.EPOCH_SECONDS:
            mapping.scale = 0.001
        elif source == TimestampFormat.EPOCH_SECONDS and target == TimestampFormat.EPOCH_MILLIS:
            mapping.scale = 1000.0
        elif source == TimestampFormat.RELATIVE_SECONDS and target == TimestampFormat.EPOCH_SECONDS:
            mapping.offset = stream_info["base_timestamp"]
        elif source == TimestampFormat.RTMP_TIMESTAMP and target == TimestampFormat.EPOCH_SECONDS:
    ...
    async def _check_sync_opportunity(
        self,
        stream_id: str,
        frame: VideoFrame
    ) -> float:
        ...
        if total_streams > 0:
            for other_stream_id, buffer in self._sync_buffer.items():
                ...
                close_frames = [
                    f for f in buffer
                    if abs(f.timestamp - frame.timestamp) < self.config.sync_window_ms / 1000.0
                ]
    ...
    async def _create_sync_point(
        ...
        for other_stream_id, buffer in self._sync_buffer.items():
            ...
            closest_frame = min(
                buffer,
                key=lambda f: abs(f.timestamp - frame.timestamp),
                default=None
            )
            ...
            if closest_frame:
                ...
                if time_diff < self.config.sync_window_ms / 1000.0:
                    sync_point.stream_timestamps[other_stream_id] = closest_frame.timestamp
    ...
    async def _sync_update_loop(self) -> None:
        ...
        while True:
            try:
                ...
                self._sync_points = [
                    sp for sp in self._sync_points
