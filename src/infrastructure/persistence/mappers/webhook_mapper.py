...
class WebhookMapper(Mapper[DomainWebhook, PersistenceWebhook]):
    ...
    def to_domain(self, model: PersistenceWebhook) -> DomainWebhook:
        ...
        events = (
            [WebhookEvent(e) for e in json.loads(model.events)] if model.events else []
        )
        custom_headers = (
            json.loads(model.custom_headers) if model.custom_headers else {}
        )
    ...
    def to_persistence(self, entity: DomainWebhook) -> PersistenceWebhook:
        ...
        model.custom_headers = (
            json.dumps(entity.custom_headers) if entity.custom_headers else "{}"
        )
