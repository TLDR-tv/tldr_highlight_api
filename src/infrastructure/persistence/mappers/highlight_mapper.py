...
from src.infrastructure.persistence.models.highlight import (
    Highlight as PersistenceHighlight,
)
...
class HighlightMapper(Mapper[DomainHighlight, PersistenceHighlight]):
    ...
    def to_domain(self, model: PersistenceHighlight) -> DomainHighlight:
        ...
        # Parse analysis data
        video_analysis = (
            json.loads(model.video_analysis) if model.video_analysis else {}
        )
        audio_analysis = (
            json.loads(model.audio_analysis) if model.audio_analysis else {}
        )
    ...
    def to_persistence(self, entity: DomainHighlight) -> PersistenceHighlight:
        ...
        model.thumbnail_url = (
            entity.thumbnail_url.value if entity.thumbnail_url else None
        )
        ...
        model.video_analysis = (
            json.dumps(entity.video_analysis) if entity.video_analysis else None
        )
        model.audio_analysis = (
            json.dumps(entity.audio_analysis) if entity.audio_analysis else None
        )
        model.chat_analysis = (
            json.dumps(entity.chat_analysis) if entity.chat_analysis else None
        )
