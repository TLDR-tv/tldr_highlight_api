...
from src.infrastructure.persistence.models.organization import (
    Organization as PersistenceOrganization,
)
...
class OrganizationMapper(Mapper[DomainOrganization, PersistenceOrganization]):
    ...
    def to_domain(self, model: PersistenceOrganization) -> DomainOrganization:
        ...
        return DomainOrganization(
            ...
            settings=settings,
            subscription_started_at=Timestamp(model.subscription_started_at)
            if model.subscription_started_at
            else None,
            subscription_ends_at=Timestamp(model.subscription_ends_at)
            if model.subscription_ends_at
            else None,
    ...
    def to_persistence(self, entity: DomainOrganization) -> PersistenceOrganization:
        ...
        model.custom_limits = (
            json.dumps(entity.custom_limits) if entity.custom_limits else None
        )
        model.settings = json.dumps(entity.settings) if entity.settings else "{}"

        # Set subscription details
        model.subscription_started_at = (
            entity.subscription_started_at.value
            if entity.subscription_started_at
            else None
        )
        model.subscription_ends_at = (
            entity.subscription_ends_at.value if entity.subscription_ends_at else None
        )
