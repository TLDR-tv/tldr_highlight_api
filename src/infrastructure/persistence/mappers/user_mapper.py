...
class UserMapper(Mapper[DomainUser, PersistenceUser]):
    ...
    def to_domain(self, model: PersistenceUser) -> DomainUser:
        ...
        return DomainUser(
            ...
            organization_ids=[org.id for org in model.owned_organizations] if model.owned_organizations else [],
            ...
            webhook_ids=[webhook.id for webhook in model.webhooks] if model.webhooks else []
        )
