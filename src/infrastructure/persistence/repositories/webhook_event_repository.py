...
from src.infrastructure.persistence.mappers.webhook_event_mapper import WebhookEventMapper
...
class WebhookEventRepositoryImpl(
    ...
    async def get_failed_events_for_retry(
        ...
        stmt = select(PersistenceWebhookEvent).where(
            ...
            PersistenceWebhookEvent.updated_at.asc()
        ).limit(limit)
    ...
    async def exists_by_external_id(self, external_event_id: str, platform: str) -> bool:
        ...
        stmt = select(PersistenceWebhookEvent.id).where(
            and_(
                PersistenceWebhookEvent.event_id == external_event_id,
                PersistenceWebhookEvent.platform == platform
            )
    ...
    async def get_events_by_stream(self, stream_id: int) -> List[DomainWebhookEvent]:
        ...
        stmt = select(PersistenceWebhookEvent).where(
            PersistenceWebhookEvent.stream_id == stream_id
