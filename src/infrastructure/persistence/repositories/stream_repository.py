...
from src.domain.repositories.stream_repository import StreamRepository as IStreamRepository
...
class StreamRepository(BaseRepository[Stream, StreamModel, int], IStreamRepository):
    ...
    async def get_by_organization(self, organization_id: int,
        ...
        stmt = select(StreamModel).join(
            UserModel, StreamModel.user_id == UserModel.id
        ).join(
    ...
    async def get_by_platform(self, platform: StreamPlatform,
                            limit: int = 100,
                            offset: int = 0) -> List[Stream]:
        ...
        """
        stmt = select(StreamModel).where(
            StreamModel.platform == platform.value
        ).order_by(StreamModel.created_at.desc()).limit(limit).offset(offset)
    ...
    async def get_by_date_range(self, start: Timestamp, end: Timestamp,
                               user_id: Optional[int] = None) -> List[Stream]:
    ...
    async def count_by_status(self, status: StreamStatus,
        ...
        """
        stmt = select(func.count()).select_from(StreamModel).where(
            StreamModel.status == status.value
        )
    ...
    async def get_with_highlights(self, stream_id: int) -> Optional[Stream]:
        ...
        """
        stmt = select(StreamModel).where(
            StreamModel.id == stream_id
        ).options(
            selectinload(StreamModel.highlights)
        )
    ...
    async def get_processing_stats(self, user_id: int) -> dict:
        ...
        total_processing_stmt = select(
            func.sum(StreamModel.duration_seconds)
        ).where(
            and_(
                StreamModel.user_id == user_id,
                StreamModel.status == StreamStatus.COMPLETED.value,
