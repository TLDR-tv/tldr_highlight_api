...
class BatchRepository(BaseRepository[Batch, BatchModel, int], IBatchRepository):
    ...
    def __init__(self, session):
        ...
        super().__init__(session=session, model_class=BatchModel, mapper=BatchMapper())
    ...
    async def get_by_user(
        ...
        """
        stmt = select(BatchModel).where(BatchModel.user_id == user_id)
    ...
    async def get_active_batches(self) -> List[Batch]:
        ...
        stmt = (
            ...
            .order_by(BatchModel.created_at.asc())
        )  # FIFO processing
    ...
    async def get_by_date_range(
        ...
        stmt = select(BatchModel).where(
            and_(
                BatchModel.created_at >= start.value, BatchModel.created_at <= end.value
            )
        )
    ...
    async def count_by_status(
        self, status: BatchStatus, user_id: Optional[int] = None
    ) -> int:
        ...
        stmt = (
            ...
            .where(BatchModel.status == status.value)
        )
    ...
    async def get_processing_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        avg_processing_stmt = select(
            ...
            and_(
                ...
                BatchModel.status.in_(
                    [BatchStatus.COMPLETED.value, BatchStatus.PARTIALLY_FAILED.value]
                ),
        ...
        success_stmt = select(
            func.avg(
                BatchModel.successful_items / func.nullif(BatchModel.total_items, 0)
        ...
        ).where(and_(BatchModel.user_id == user_id, BatchModel.total_items > 0))
    ...
    async def get_failed_items_summary(
        ...
        stmt = (
            select(
                BatchModel.id, BatchModel.name, BatchModel.items, BatchModel.created_at
            )
            .where(and_(BatchModel.user_id == user_id, BatchModel.failed_items > 0))
    ...
    async def get_batch_queue_stats(self) -> Dict[str, Any]:
        ...
        oldest_pending_stmt = select(func.min(BatchModel.created_at)).where(
        ...
        avg_queue_time_stmt = select(
            ...
            and_(BatchModel.started_at.isnot(None), BatchModel.created_at.isnot(None))
        )
    ...
    async def get_batches_by_success_rate(
        self, min_success_rate: float = 0.8, user_id: Optional[int] = None
    ) -> List[Batch]:
        ...
        stmt = select(BatchModel).where(
            and_(
                ...
                (BatchModel.successful_items / BatchModel.total_items)
                >= min_success_rate,
            )
        )
    ...
    async def get_user_batch_analytics(
        ...
        stmt = select(BatchModel).where(
            and_(BatchModel.user_id == user_id, BatchModel.created_at >= start_date)
        )
        ...
        for batch_model in batches:
            ...
            if batch_model.started_at and batch_model.completed_at:
                processing_time = (
                    batch_model.completed_at - batch_model.started_at
                ).total_seconds()
