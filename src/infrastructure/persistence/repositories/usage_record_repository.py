...
from src.domain.repositories.usage_record_repository import UsageRecordRepository as IUsageRecordRepository
from src.domain.entities.usage_record import UsageRecord, UsageType
from src.domain.value_objects.timestamp import Timestamp
from src.domain.exceptions import EntityNotFoundError
from src.infrastructure.persistence.repositories.base_repository import BaseRepository
from src.infrastructure.persistence.models.usage_record import UsageRecord as UsageRecordModel
from src.infrastructure.persistence.mappers.usage_record_mapper import UsageRecordMapper


class UsageRecordRepository(BaseRepository[UsageRecord, UsageRecordModel, int], IUsageRecordRepository):
    ...
    async def get_usage_by_api_key(self, api_key_id: int,
        ...
        """
        stmt = select(UsageRecordModel).where(
            UsageRecordModel.api_key_id == api_key_id
        )
    ...
    async def get_usage_analytics(self, user_id: Optional[int] = None,
    ...
    async def get_top_consumers(self, usage_type: UsageType,
                              days: int = 30,
        ...
        stmt = select(
            ...
            func.sum(UsageRecordModel.quantity).desc()
        ).limit(limit)
    ...
    async def cleanup_old_records(self, older_than: Timestamp,
                                keep_billable: bool = True) -> int:
    ...
    async def get_billing_summary(self, user_id: int,
                                billing_period_start: Timestamp,
                                billing_period_end: Timestamp) -> Dict[str, Any]:
