...
from src.domain.repositories.dimension_set_repository import DimensionSetRepository as IDimensionSetRepository
from src.domain.entities.dimension_set import DimensionSet
from src.domain.exceptions import EntityNotFoundError
from src.infrastructure.persistence.repositories.base_repository import BaseRepository
from src.infrastructure.persistence.models.dimension_set import DimensionSet as DimensionSetModel
from src.infrastructure.persistence.mappers.dimension_set_mapper import DimensionSetMapper


class DimensionSetRepository(BaseRepository[DimensionSet, DimensionSetModel, int], IDimensionSetRepository):
    """Concrete implementation of DimensionSetRepository using SQLAlchemy."""
    ...
    def __init__(self, session):
        ...
        super().__init__(
            session=session,
            model_class=DimensionSetModel,
            mapper=DimensionSetMapper()
        )
    ...
    async def delete(self, dimension_set_id: int) -> bool:
        ...
        """
        stmt = select(DimensionSetModel).where(
            DimensionSetModel.id == dimension_set_id
        )
