...
class WebhookRepository(BaseRepository[Webhook, WebhookModel, int], IWebhookRepository):
    ...
    def __init__(self, session):
        ...
        super().__init__(
            session=session, model_class=WebhookModel, mapper=WebhookMapper()
        )
    ...
    async def get_by_user(
        ...
        """
        stmt = select(WebhookModel).where(WebhookModel.user_id == user_id)
    ...
    async def get_by_url(
        ...
        """
        stmt = select(WebhookModel).where(WebhookModel.url == url.value)
        ...
        stmt = (
            ...
            .where(
                and_(
                    ...
                    func.json_extract(WebhookModel.events, "$").contains(
                        f'"{event.value}"'
                    ),
                )
            )
    ...
    async def get_failed_webhooks(self, failure_threshold: int = 5) -> List[Webhook]:
        ...
        stmt = (
            select(WebhookModel)
            .where(WebhookModel.consecutive_failures >= failure_threshold)
    ...
    async def count_by_status(self, user_id: Optional[int] = None) -> Dict[str, int]:
        ...
        """
        stmt = select(WebhookModel.status, func.count()).group_by(WebhookModel.status)
    ...
    async def get_delivery_stats(self, webhook_id: int) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "failed_deliveries": webhook.total_deliveries
            - webhook.successful_deliveries,
            "consecutive_failures": webhook.consecutive_failures,
            "success_rate": webhook.success_rate,
            "last_delivery": {
                "delivered_at": webhook.last_delivery.delivered_at.iso_string
                if webhook.last_delivery
                else None,
                "status_code": webhook.last_delivery.status_code
                if webhook.last_delivery
                else None,
                "response_time_ms": webhook.last_delivery.response_time_ms
                if webhook.last_delivery
                else None,
                "was_successful": webhook.last_delivery.is_successful
                if webhook.last_delivery
                else None,
                "error_message": webhook.last_delivery.error_message
                if webhook.last_delivery
                else None,
            }
            if webhook.last_delivery
    ...
    async def get_webhooks_by_organization(
        self, organization_id: int, status: Optional[WebhookStatus] = None
    ) -> List[Webhook]:
    ...
    async def get_webhooks_by_success_rate(
        self, min_success_rate: float = 0.8, min_deliveries: int = 10
    ) -> List[Webhook]:
        ...
        stmt = (
            ...
            .where(
                and_(
                    ...
                    (WebhookModel.successful_deliveries / WebhookModel.total_deliveries)
                    >= min_success_rate,
                )
            )
            .order_by(
    ...
    async def bulk_update_status(
        self, webhook_ids: List[int], new_status: WebhookStatus
    ) -> int:
    ...
    async def get_webhook_usage_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        stmt = select(
            func.avg(
                WebhookModel.successful_deliveries
                / func.nullif(WebhookModel.total_deliveries, 0)
