...
class HighlightRepository(BaseRepository[Highlight, HighlightModel, int], IHighlightRepository):
    ...
    async def get_by_stream(self, stream_id: int,
        ...
        stmt = select(HighlightModel).where(
            HighlightModel.stream_id == stream_id
        )
    ...
    async def get_trending(self, time_window: Timestamp,
        ...
        stmt = select(
            ...
            (
                ...
                engagement_weight * func.coalesce(HighlightModel.viewer_engagement, 0.0) +
                recency_weight * func.exp(
                    -func.extract('epoch', func.now() - HighlightModel.created_at) / 86400.0
                )
            ).label('trending_score')
        ).where(
            HighlightModel.created_at >= time_window.value
        ).order_by(
            text('trending_score DESC')
        ).limit(limit)
    ...
    async def get_statistics(self, user_id: int) -> Dict[str, Any]:
        ...
        avg_confidence_stmt = select(
            func.avg(HighlightModel.confidence_score)
        ).select_from(
            HighlightModel
        ).join(
            StreamModel, HighlightModel.stream_id == StreamModel.id
    ...
    async def get_by_stream_with_pagination(self, stream_id: int,
                                          limit: int = 50,
                                          offset: int = 0,
                                          order_by: str = "confidence") -> Dict[str, Any]:
        ...
        # Get total count
        count_stmt = select(func.count()).select_from(HighlightModel).where(
            HighlightModel.stream_id == stream_id
        )
