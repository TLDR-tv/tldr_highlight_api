...
from src.domain.repositories.organization_repository import OrganizationRepository as IOrganizationRepository
...
from src.infrastructure.persistence.models.organization import Organization as OrganizationModel
...
class OrganizationRepository(BaseRepository[Organization, OrganizationModel, int], IOrganizationRepository):
    ...
    async def get_by_member(self, user_id: int) -> List[Organization]:
        ...
        stmt = select(OrganizationModel).join(
            UserModel, OrganizationModel.members
        ).where(
    ...
    async def get_by_name(self, name: CompanyName) -> Optional[Organization]:
        ...
        """
        stmt = select(OrganizationModel).where(
            OrganizationModel.name == name.value
        )
    ...
    async def get_by_plan_type(self, plan_type: PlanType) -> List[Organization]:
        ...
        """
        stmt = select(OrganizationModel).where(
            OrganizationModel.plan_type == plan_type.value
        ).order_by(OrganizationModel.created_at.desc())
    ...
    async def count_by_plan_type(self) -> Dict[PlanType, int]:
        ...
        stmt = select(
            ...
            func.count()
        ).group_by(OrganizationModel.plan_type)
    ...
    async def get_organization_analytics(self, organization_id: int, 
        ...
        return {
            ...
            'subscription_status': 'active' if org.is_subscription_active else 'inactive',
            'member_stats': {
                'current_members': current_members,
                'max_members': plan_limits.team_members,
                'utilization_percentage': (current_members / plan_limits.team_members) * 100
            },
            'plan_limits': {
            ...
            'subscription_started_at': org.subscription_started_at.iso_string if org.subscription_started_at else None,
            'subscription_ends_at': org.subscription_ends_at.iso_string if org.subscription_ends_at else None
        }
    
    async def get_organizations_by_usage(self, min_usage_minutes: int = 0,
                                       period_days: int = 30) -> List[Organization]:
    ...
    async def bulk_update_subscriptions(self, updates: List[Dict]) -> int:
        ...
        for update_data in updates:
            ...
            updated_org = Organization(
                ...
                subscription_started_at=Timestamp(update_data['subscription_started_at']) if update_data.get('subscription_started_at') else org.subscription_started_at,
                subscription_ends_at=Timestamp(update_data['subscription_ends_at']) if update_data.get('subscription_ends_at') else org.subscription_ends_at,
