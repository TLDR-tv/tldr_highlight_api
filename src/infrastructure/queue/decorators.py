...
def priority_task(priority: int = 5, queue: str = "default") -> Callable:
...
def batch_task(batch_size: int = 100, priority: int = 3) -> Callable:
...
def exclusive_task(
    lock_timeout: int = 3600, lock_key_prefix: str = "exclusive"
) -> Callable:
    ...
    def decorator(func: Callable[P, T]) -> Callable[P, T]:
        ...
        async def wrapper(self, *args: P.args, **kwargs: P.kwargs) -> T:
            ...
            # Try to acquire lock
            acquired = await cache.set(lock_key, "locked", ttl=lock_timeout, nx=True)
