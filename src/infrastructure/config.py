...
class Settings(BaseSettings):
    ...
    s3_secret_access_key: SecretStr = Field(default="", description="AWS secret access key")
    s3_region: str = Field(default="us-east-1", description="AWS region")
    s3_highlights_bucket: str = Field(default="tldr-highlights", description="S3 bucket for highlights")
    s3_thumbnails_bucket: str = Field(default="tldr-thumbnails", description="S3 bucket for thumbnails")
    s3_temp_bucket: str = Field(default="tldr-temp", description="S3 bucket for temporary files")
    ...
    google_gemini_api_key: SecretStr = Field(default="", description="Google Gemini API key")
    twitch_client_id: str = Field(default="", description="Twitch client ID")
    twitch_client_secret: SecretStr = Field(default="", description="Twitch client secret")
    ...
    logfire_project_name: str = Field(
        default="tldr-highlight-api",
        description="Logfire project name"
    )
    logfire_api_key: Optional[SecretStr] = Field(
        default=None,
        description="Logfire API key (optional for local development)"
    )
    logfire_environment: Optional[str] = Field(
        default=None,
        description="Logfire environment (defaults to app environment)"
    )
    logfire_service_name: str = Field(
        default="tldr-api",
        description="Service name for Logfire"
    )
    logfire_service_version: Optional[str] = Field(
        default=None,
        description="Service version (defaults to app version)"
    )
    logfire_console_enabled: bool = Field(
        default=True,
        description="Enable Logfire console output"
    )
    logfire_log_level: str = Field(
        default="INFO",
        description="Logfire log level"
    )
    logfire_capture_headers: bool = Field(
        default=True,
        description="Capture HTTP headers in Logfire"
    )
    ...
    logfire_sql_enabled: bool = Field(
        default=True,
        description="Enable SQL query logging in Logfire"
    )
    logfire_redis_enabled: bool = Field(
        default=True,
        description="Enable Redis command logging in Logfire"
    )
    logfire_celery_enabled: bool = Field(
        default=True,
        description="Enable Celery task logging in Logfire"
    )
    ...
    # Feature flags
    enable_webhooks: bool = Field(default=True, description="Enable webhook functionality")
    enable_batch_processing: bool = Field(default=True, description="Enable batch processing")
    enable_real_time_processing: bool = Field(default=True, description="Enable real-time stream processing")
