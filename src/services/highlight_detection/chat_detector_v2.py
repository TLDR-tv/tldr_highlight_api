...
from .chat_detector import ChatDetectionConfig, ChatMessage as OldChatMessage, ChatWindow
...
class ChatMessageAdapter:
    ...
    @staticmethod
    def old_to_new(old_msg: OldChatMessage, platform: str = "unknown") -> NewChatMessage:
...
class EnhancedChatDetector(BaseDetector):
    ...
    async def _detect_features(
        ...
        """
        chat_config = (
            config if isinstance(config, EnhancedChatDetectionConfig) else self.chat_config
        )
        ...
        try:
            ...
            if not messages:
                self.logger.warning(f"No valid messages in segment {segment.segment_id}")
                return []
            ...
            velocity_metrics = self.sentiment_analyzer.temporal_analyzer.calculate_velocity_metrics(
                end_time
            )
            
            # Get event impact
            event_impact = self.sentiment_analyzer.event_tracker.get_current_impact(end_time)
            ...
            sentiment_score = self._calculate_sentiment_score(window_sentiment, chat_config)
            velocity_score = self._calculate_velocity_score(velocity_metrics, chat_config)
            event_score = event_impact * chat_config.event_impact_weight
            
            # Combine scores
            final_score = (
                sentiment_score * (1 - chat_config.velocity_spike_weight - chat_config.event_impact_weight) +
            ...
            # Check thresholds
            if (
                final_score < chat_config.min_score or
                highlight_confidence < chat_config.min_confidence
            ):
                return []
    ...
    def _calculate_sentiment_score(
        ...
        sentiment_score = (
            category_weight * 0.5 +
            window.intensity * 0.3 +
            window.confidence * 0.2
        )
    ...
    def _calculate_velocity_score(
        ...
        if velocity.spike_detected and velocity.spike_intensity >= config.min_velocity_spike_intensity:
