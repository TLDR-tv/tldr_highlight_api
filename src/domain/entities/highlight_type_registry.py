...
class HighlightTypeDefinition:
    ...
    required_dimensions: List[str] = field(default_factory=list)  # Must have these dimensions
    required_dimension_scores: Dict[str, float] = field(default_factory=dict)  # Min scores per dimension
    ...
    def matches_criteria(self, score: float, dimension_scores: Dict[str, float]) -> bool:
...
class HighlightTypeRegistry(Entity[int]):
    ...
    def update_type(self, type_id: str, updates: Dict[str, Any]) -> None:
        ...
        allowed_fields = {
            'name', 'description', 'min_score_threshold', 
            'required_dimensions', 'required_dimension_scores',
            'priority', 'is_exclusive', 'auto_assign',
            'color', 'icon', 'tags', 'is_active'
        }
    ...
    def determine_types(
        ...
        for type_id, type_def in self.types.items():
            ...
            if type_def.matches_criteria(score, dimension_scores):
                matching_types.append((type_def.priority, type_id, type_def.is_exclusive))
    ...
    def get_active_types(self) -> List[HighlightTypeDefinition]:
        ...
        """
        active_types = [
            type_def for type_def in self.types.values()
            if type_def.is_active
        ]
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if self.default_type_id not in self.types:
            errors.append(f"Default type '{self.default_type_id}' not found in registry")
    ...
    def clone(self, new_organization_id: int, new_user_id: int) -> "HighlightTypeRegistry":
    ...
    @classmethod
    def create_gaming_registry(cls, organization_id: int, user_id: int) -> "HighlightTypeRegistry":
    ...
    @classmethod
    def create_educational_registry(cls, organization_id: int, user_id: int) -> "HighlightTypeRegistry":
