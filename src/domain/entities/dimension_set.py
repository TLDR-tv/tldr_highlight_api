...
class DimensionSet(Entity[int]):
    ...
    allow_partial_scoring: bool = (
        True  # Allow highlights even if some dimensions can't be scored
    )
    ...
    def __post_init__(self):
        ...
        if not self.dimension_weights and self.dimensions:
            self.dimension_weights = {
                dim_id: dim.default_weight for dim_id, dim in self.dimensions.items()
            }
    ...
    def update_weight(self, dimension_id: str, new_weight: float) -> None:
        ...
        if not 0.0 <= new_weight <= 1.0:
            raise InvalidValueError(
                f"Weight must be between 0.0 and 1.0, got {new_weight}"
            )
    ...
    def update_all_weights(self, weights: Dict[str, float]) -> None:
        ...
        if invalid_dims:
            raise BusinessRuleViolation(f"Unknown dimensions: {invalid_dims}")
    ...
    def calculate_score(self, dimension_scores: Dict[str, float]) -> float:
        ...
        if (
            len(scored_dimensions) < self.minimum_dimensions_required
            and not self.allow_partial_scoring
        ):
            raise BusinessRuleViolation(
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if self.weight_normalization:
            ...
            if abs(total_weight - 1.0) > 0.01:
                errors.append(
                    f"Weights sum to {total_weight:.3f} but should sum to 1.0"
                )

        # Check for zero weights
        zero_weight_dims = [
            dim_id for dim_id, weight in self.dimension_weights.items() if weight == 0.0
        ]
        if zero_weight_dims:
