...
def validate_calibration_effectiveness(
    ...
    for dim_id in all_dimensions:
        ...
        effectiveness_metrics[dim_id] = {
            ...
            "std_error": std_error,
            "effectiveness": 1.0
            - (mean_error + std_error) / 2.0,  # Simple effectiveness score
        }
...
def merge_calibration_profiles(
    profiles: List[CalibrationProfile], weights: Optional[List[float]] = None
) -> CalibrationProfile:
...
def detect_calibration_drift(
    ...
    for dim_id in current_profile.scale_factors:
        ...
        calibrated_scores = [
            (score * current_profile.scale_factors[dim_id])
            + current_profile.offsets[dim_id]
            for score in scores
        ]
