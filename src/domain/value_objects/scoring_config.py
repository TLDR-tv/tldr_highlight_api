...
class DimensionWeights:
    ...
    def __post_init__(self) -> None:
        ...
        total = (
            ...
            self.visual_spectacle + self.emotional_intensity + 
            self.narrative_value + self.timing_importance + self.momentum_shift
        )
...
class ScoreThresholds:
    ...
    def __post_init__(self) -> None:
        ...
        all_thresholds = [
            self.minimum_viable, self.good_quality, self.exceptional, self.legendary,
            self.skill_play, self.clutch_moment, self.comeback, self.rare_event,
            self.perfect_execution, self.wake_word_boost, self.manual_trigger
        ]
        
        for threshold in all_thresholds:
            if not (0.0 <= threshold <= 1.0):
                raise ValueError(f"All thresholds must be between 0.0 and 1.0, got {threshold}")
...
class ScoringDimensions:
    ...
    def __post_init__(self) -> None:
        ...
        dimensions = [
            self.skill_execution, self.game_impact, self.rarity,
            self.visual_spectacle, self.emotional_intensity, self.narrative_value,
            self.timing_importance, self.momentum_shift
        ]
        
        for dim in dimensions:
            if not (0.0 <= dim <= 1.0):
                raise ValueError(f"All dimension scores must be between 0.0 and 1.0, got {dim}")
