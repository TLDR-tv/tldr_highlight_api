...
class DimensionDefinition:
    ...
    applicable_modalities: List[str] = field(default_factory=lambda: ["video", "audio", "text"])
    ...
    def __post_init__(self):
        ...
        if not self.id or not self.id.replace("_", "").isalnum():
            raise InvalidValueError(f"Dimension ID must be alphanumeric with underscores, got '{self.id}'")
        
        # Validate weight
        if not 0.0 <= self.default_weight <= 1.0:
            raise InvalidValueError(f"Default weight must be between 0.0 and 1.0, got {self.default_weight}")
        
        # Validate value range
        if self.min_value >= self.max_value:
            raise InvalidValueError(f"Min value ({self.min_value}) must be less than max value ({self.max_value})")
        
        if not self.min_value <= self.threshold <= self.max_value:
            raise InvalidValueError(f"Threshold ({self.threshold}) must be between min ({self.min_value}) and max ({self.max_value})")
        ...
        for example in self.examples:
            ...
            if "value" not in example or "description" not in example:
                raise InvalidValueError("Examples must have 'value' and 'description' keys")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DimensionDefinition":
        ...
        return cls(
            ...
            applicable_modalities=data.get("applicable_modalities", ["video", "audio", "text"]),
    ...
    def create_skill_dimension(
        ...
        return cls(
            ...
            default_weight=0.2,
            scoring_prompt=skill_prompt or f"Evaluate the level of {name.lower()} demonstrated. Consider technical difficulty, precision, and execution quality.",
            category="technical",
            examples=[
                {"value": 0.9, "description": "Expert-level execution with perfect precision"},
                {"value": 0.5, "description": "Competent performance with minor imperfections"},
    ...
    def create_emotional_dimension(
        ...
        return cls(
            ...
            dimension_type=DimensionType.NUMERIC,
            default_weight=0.15,
            scoring_prompt=emotion_prompt or f"Assess the {name.lower()} of this moment. Consider audience reaction potential and emotional resonance.",
            category="emotional",
    ...
    def create_contextual_dimension(
        ...
        return cls(
            ...
            dimension_type=DimensionType.NUMERIC,
            default_weight=0.1,
            scoring_prompt=context_prompt or f"Evaluate the {name.lower()} given the current context and situation.",
            category="contextual",
    ...
    def create_binary_dimension(
        ...
        return cls(
            ...
            max_value=1.0,
            threshold=0.5,
            scoring_prompt=detection_prompt or f"Determine if this moment exhibits {name.lower()} (1 for yes, 0 for no).",
            category="binary",
            examples=[
