...
class ConfigurablePromptTemplate:
    ...
    def __post_init__(self) -> None:
        ...
        total_length = (
            ...
            + len(self.scoring_instructions)
            + len(self.context_instructions)
        )

        if total_length > self.max_length:
            raise ValueError(
                f"Total prompt length {total_length} exceeds maximum {self.max_length}"
            )
...
Consider both the athletic achievement and entertainment value.
    ...
    def from_b2c_config(
        ...
        analysis_prompt = b2c_config.get("analysis_prompt", "") or b2c_config.get(
            "prompt_template", ""
        )
