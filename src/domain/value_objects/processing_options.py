...
class ProcessingOptions:
    ...
    enabled_modalities: Set[str] = field(default_factory=lambda: {"video", "audio", "text"})
    ...
    def __post_init__(self):
        ...
        # Validate thresholds
        thresholds = [self.min_confidence_threshold, self.target_confidence_threshold, self.exceptional_threshold]
        for threshold in thresholds:
            if not 0.0 <= threshold <= 1.0:
                raise InvalidValueError(f"Confidence thresholds must be between 0.0 and 1.0")
        
        if not (self.min_confidence_threshold <= self.target_confidence_threshold <= self.exceptional_threshold):
            raise InvalidValueError("Thresholds must be in ascending order: min <= target <= exceptional")
        
        # Validate duration constraints
        if self.min_highlight_duration < 0:
            raise InvalidValueError(f"Minimum highlight duration cannot be negative")
        ...
        if not self.min_highlight_duration <= self.typical_highlight_duration <= self.max_highlight_duration:
        ...
        if self.fusion_strategy == FusionStrategy.WEIGHTED:
            total_weight = sum(self.modality_weights.get(m, 0) for m in self.enabled_modalities)
            if abs(total_weight - 1.0) > 0.01:
                raise InvalidValueError(f"Modality weights must sum to 1.0, got {total_weight}")
    ...
    def with_strategy(self, detection: Optional[DetectionStrategy] = None, 
                     fusion: Optional[FusionStrategy] = None) -> "ProcessingOptions":
    ...
    def is_high_quality(self) -> bool:
        ...
        return (self.processing_priority == "quality" and 
    ...
    def requires_dimension_set(self) -> bool:
        ...
        return self.detection_strategy in [DetectionStrategy.AI_ONLY, DetectionStrategy.HYBRID]
