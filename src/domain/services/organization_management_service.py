...
class OrganizationManagementService(BaseDomainService):
    ...
    async def add_member(
        ...
        if user_orgs:
            raise BusinessRuleViolation("User is already a member of another organization")
    ...
    async def upgrade_plan(
        ...
        if plan_order.get(new_plan, 0) < plan_order.get(org.plan_type, 0):
            raise BusinessRuleViolation("Plan downgrades must be handled through support")
    ...
    async def check_and_enforce_quotas(
        self,
        organization_id: int
    ) -> Dict[str, Any]:
        ...
        # API Keys
        api_keys = await self.api_key_repo.get_by_user(
            org.owner_id, active_only=True
        )
        ...
        quota_status = {
            ...
            "monthly_processing_minutes": {
                ...
                "available": max(0, plan_limits.monthly_processing_minutes - processing_minutes),
                "at_limit": processing_minutes >= plan_limits.monthly_processing_minutes
            },
            "team_members": {
        ...
        for member_id in [org.owner_id] + org.member_ids:
            ...
            if user:
                ...
                member_activity.append({
                    ...
                    "last_active": max(
                        (s.created_at for s in user_streams),
                        default=None
                    )
                })
    ...
    async def deactivate_expired_subscriptions(self) -> int:
        ...
        for org in expired_orgs:
            if org.subscription_ends_at and org.subscription_ends_at.is_before(Timestamp.now()):
