...
from src.domain.entities.webhook import Webhook, WebhookEvent, WebhookDelivery, WebhookStatus
...
class WebhookDeliveryService(BaseDomainService):
    ...
    async def trigger_event(
        ...
        if not webhooks:
            self.logger.info(f"No active webhooks for event {event.value} and user {user_id}")
            return []
    ...
    async def cleanup_failed_webhooks(
        self,
        ...
        if event == WebhookEvent.STREAM_STARTED:
            ...
            if stream:
                base_payload["data"] = {
                    ...
                    "started_at": stream.started_at.iso_string if stream.started_at else None
                }
            ...
            if stream:
                ...
                base_payload["data"] = {
                    ...
                    "completed_at": stream.completed_at.iso_string if stream.completed_at else None
                }
            ...
            if batch:
                base_payload["data"] = {
                    ...
                    "completed_at": batch.completed_at.iso_string if batch.completed_at else None
                }
