...
from ..entities.stream import Stream, StreamStatus
...
class B2BStreamAgent:
    ...
    def _create_highlight_candidate(
        ...
        try:
            ...
            if not self.agent_config.score_thresholds.meets_threshold(final_score, highlight_type):
                return None
    ...
    async def should_create_highlight(self, candidate: HighlightCandidate) -> bool:
        ...
        try:
            ...
            recent_window = [
                h for h in self.recent_highlights
    ...
    def _calculate_similarity(
        self,
        candidate1: HighlightCandidate,
        candidate2: HighlightCandidate
    ) -> float:
        ...
        if desc1_words or desc2_words:
            word_similarity = len(desc1_words & desc2_words) / len(desc1_words | desc2_words)
        else:
            word_similarity = 0
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "processing_stats": {
                ...
                "highlights_created": self.highlights_created,
                "success_rate": self.highlights_created / max(1, self.candidates_evaluated),
                "processing_rate": self.segments_processed / max(1, uptime / 60),  # per minute
            },
            "quality_metrics": {
                "avg_confidence": sum(h.confidence for h in self.recent_highlights) / max(1, len(self.recent_highlights)),
                "avg_score": sum(h.final_score for h in self.recent_highlights) / max(1, len(self.recent_highlights)),
    ...
    def _get_score_distribution(self) -> Dict[str, int]:
        ...
        distribution = {"0.0-0.5": 0, "0.5-0.7": 0, "0.7-0.8": 0, "0.8-0.9": 0, "0.9-1.0": 0}
