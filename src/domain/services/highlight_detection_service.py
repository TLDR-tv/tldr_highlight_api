...
from src.domain.services.analysis_strategies import (
    AnalysisStrategy,
    AnalysisSegment,
    AnalysisResult,
)
...
class HighlightDetectionService(BaseDomainService):
    ...
    async def process_stream_segment(
        ...
        if processing_options.dimension_set_id:
            dimension_set = await self.dimension_set_repo.get(
                processing_options.dimension_set_id
            )
            if not dimension_set:
                raise EntityNotFoundError(
                    f"DimensionSet {processing_options.dimension_set_id} not found"
                )

        # Load type registry if specified
        type_registry = None
        if processing_options.type_registry_id:
            type_registry = await self.type_registry_repo.get(
                processing_options.type_registry_id
            )
            if not type_registry:
                raise EntityNotFoundError(
                    f"TypeRegistry {processing_options.type_registry_id} not found"
                )
        ...
        strategy = self.analysis_strategies.get(
            processing_options.detection_strategy.value
        )
        if not strategy:
            raise BusinessRuleViolation(
                f"Unknown analysis strategy: {processing_options.detection_strategy}"
            )
        ...
        if self._is_highlight(analysis_result, processing_options):
            highlight = await self._create_highlight(
                stream_id, analysis_segment, analysis_result, type_registry
            )
            return [highlight]
    ...
    async def detect_highlights_batch(
        ...
        for segment_data in segments:
            segment_highlights = await self.process_stream_segment(
                stream_id, segment_data, processing_options
            )
            highlights.extend(segment_highlights)

        # Post-process to merge adjacent highlights
        if highlights:
            highlights = await self._post_process_highlights(
                highlights, processing_options
            )
    ...
    def _consensus_fusion(
        self, modality_scores: Dict[str, Dict[str, float]]
    ) -> Dict[str, float]:
        ...
        for dimension in common_dimensions:
            min_score = min(
                scores.get(dimension, 0.0) for scores in modality_scores.values()
            )
    ...
    def _is_highlight(
        self, analysis_result: AnalysisResult, options: ProcessingOptions
    ) -> bool:
        ...
        high_scoring_dimensions = sum(
            1
            for score in analysis_result.dimension_scores.values()
    ...
    async def _create_highlight(
        ...
        if type_registry:
            highlight_types = type_registry.determine_types(
                analysis_result.dimension_scores, analysis_result.metadata
            )
    ...
    def _generate_title(
        self, analysis_result: AnalysisResult, highlight_types: List[str]
    ) -> str:
        ...
        if analysis_result.dimension_scores:
            top_dimension = max(
                analysis_result.dimension_scores.items(), key=lambda x: x[1]
            )[0]
    ...
    def _generate_description(self, analysis_result: AnalysisResult) -> str:
        ...
        return (
            ". ".join(descriptions)
            if descriptions
            else "Automatically detected highlight"
        )

    async def _post_process_highlights(
        self, highlights: List[Highlight], options: ProcessingOptions
    ) -> List[Highlight]:
