...
class RuleBasedAnalysisStrategy(AnalysisStrategy):
    ...
    async def analyze(self, segment: AnalysisSegment) -> AnalysisResult:
        ...
        for rule_idx, rule in enumerate(self._compiled_rules):
            if self._evaluate_rule(rule, segment):
                candidate = self._create_candidate_from_rule(
                    rule, segment, rule_idx
                )
                candidates.append(candidate)
    ...
    def _evaluate_condition(
        ...
        if cond_type == "metadata":
            ...
            return self._compare_values(actual, expected, condition.get("operator", "=="))
        
        elif cond_type == "duration":
            # Check segment duration
            duration = segment.end_time - segment.start_time
            threshold = condition["threshold"]
            return self._compare_values(duration, threshold, condition.get("operator", ">="))
...
class HybridAnalysisStrategy(AnalysisStrategy):
    ...
    def __init__(
        ...
        """
        super().__init__(
            ai_strategy.dimension_set,
            ai_strategy.type_registry,
            options
        )
    ...
    def supports_modality(self, modality: str) -> bool:
        ...
        return (self.ai_strategy.supports_modality(modality) or
                self.rule_strategy.supports_modality(modality))
            ...
            for rule_candidate in rule_candidates:
                ...
                if overlap > 0.5:  # 50% overlap required
                    ...
                    merged = HighlightCandidate(
                        ...
                        start_time=max(ai_candidate.start_time, rule_candidate.start_time),
                        ...
                        confidence=min(ai_candidate.confidence, rule_candidate.confidence),
