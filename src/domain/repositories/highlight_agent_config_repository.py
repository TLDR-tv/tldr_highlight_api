...
class HighlightAgentConfigRepository(Protocol):
    ...
    async def get_by_organization(self, organization_id: int) -> List[HighlightAgentConfig]:
    ...
    async def get_active_for_organization(self, organization_id: int) -> List[HighlightAgentConfig]:
        """Get all active configurations for an organization."""
        ...
    
    async def get_default_for_content_type(self, content_type: str) -> Optional[HighlightAgentConfig]:
