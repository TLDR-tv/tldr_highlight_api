...
class HighlightFactory(factory.Factory):
    ...
    end_time = factory.LazyAttribute(lambda o: o.start_time + 30.0)  # 30 second highlight
    ...
    thumbnail_path = factory.LazyFunction(lambda: f"s3://bucket/thumbnails/{fake.uuid4()}.jpg")
...
def create_test_highlight(
    ...
    **kwargs
    ...
    if dimension_scores and "overall_score" not in kwargs:
        ...
        if total_weight > 0:
            highlight.overall_score = sum(d.score * d.confidence for d in dimension_scores) / total_weight
