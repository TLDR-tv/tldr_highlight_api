...
class TestLogfireIntegration:
    ...
    async def test_stream_processing_full_trace(
        ...
        mock_stream_repo.save = AsyncMock(return_value=Mock(
            id=123,
            user_id=1,
        ...
        mock_org_repo = Mock()
        mock_org_repo.get_by_owner = AsyncMock(return_value=[Mock(id=1, name="Test Org")])
        ...
        with patch("src.domain.services.stream_processing_service.ingest_stream_with_ffmpeg") as mock_task:
            ...
            assert mock_decorators_logfire.span.call_count >= 2  # At least use case and service
            ...
            stream_started_calls = [
                c for c in metric_calls 
                if "metric.streams.started" in str(c)
            ]
    ...
    def test_logfire_configuration_with_environment(self, mock_logfire):
        ...
        with patch("src.infrastructure.observability.logfire_setup.settings") as mock_settings:
            # Configure settings
    ...
    async def test_complete_stream_lifecycle_observability(
        ...
        mock_decorators_logfire.span.side_effect = lambda name, **kwargs: TrackingSpan(name)
        mock_metrics_logfire.info = Mock()
