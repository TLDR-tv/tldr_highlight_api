...
class TestTimelineSyncIntegration:
    ...
    @pytest.mark.asyncio
    async def test_twitch_stream_synchronization(
        self, stream_buffer_manager, frame_synchronizer
    ):
        ...
        await stream_buffer_manager.add_stream(
            stream_id, mock_adapter, StreamType.TWITCH_HLS
        )
        ...
        # Track highlights
        detected_highlights = []
        timeline_sync.add_highlight_callback(lambda h: detected_highlights.append(h))
        ...
        try:
            ...
            for i in range(10):
                user = ChatUser(
                    id=f"user_{i}", username=f"user{i}", display_name=f"User {i}"
                )
            ...
            assert (
                timeline_sync.timestamp_offset.offset_seconds == -2.0
            )  # Twitch offset
            ...
            sync_events = await timeline_sync.get_synchronized_events(
    ...
    @pytest.mark.asyncio
    async def test_youtube_vod_synchronization(
        self, stream_buffer_manager, frame_synchronizer
    ):
    ...
    @pytest.mark.asyncio
    async def test_adaptive_synchronization(
        self, stream_buffer_manager, frame_synchronizer
    ):
        ...
        with (
            patch.object(
                timeline_sync, "_get_chat_velocity_signal"
            ) as mock_chat_signal,
            patch.object(
                timeline_sync, "_get_video_activity_signal"
            ) as mock_video_signal,
        ):
...
class TestMultiStreamIntegration:
    ...
    @pytest.mark.asyncio
    async def test_multi_perspective_highlights(
        self, stream_buffer_manager, frame_synchronizer
    ):
        """Test highlight detection across multiple streams."""
        multi_sync = MultiStreamTimelineSynchronizer(
            buffer_manager=stream_buffer_manager, frame_synchronizer=frame_synchronizer
        )

        try:
            # Add multiple streams
            stream1 = await multi_sync.add_stream(
                "streamer1", ChatSourceType.TWITCH_EVENTSUB, StreamType.TWITCH_HLS
            )

            stream2 = await multi_sync.add_stream(
                "streamer2", ChatSourceType.TWITCH_EVENTSUB, StreamType.TWITCH_HLS
            )
    ...
    @pytest.mark.asyncio
    async def test_tournament_synchronization(
        self, stream_buffer_manager, frame_synchronizer
    ):
        ...
        try:
            ...
            reactions = [
                ...
                (
                    "player3",
                    1.0,
                    ["GG", "That was sick!", "Best play of the tournament!"],
                ),
            ...
            for player in players:
                ...
                assert (
                    stream_status["timestamp_offset"]["offset_seconds"] == -2.0
                )  # Twitch offset
...
async def test_real_time_highlight_generation():
    ...
    try:
        ...
        for highlight in final_highlights:
            ...
            if (
                10 <= highlight.start_timestamp <= 25
                or 40 <= highlight.start_timestamp <= 50
            ):
