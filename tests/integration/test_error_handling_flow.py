...
async def test_stream_validation_errors():
    ...
    try:
        # Negative duration
        options = ProcessingOptions(min_highlight_duration=-10.0)
...
async def test_processing_errors():
    ...
    for url, error_type in network_error_urls:
        ...
        failed_stream = processing_stream.fail_processing(
            f"{error_type}: Unable to connect to {url}"
        )
        assert failed_stream.status == StreamStatus.FAILED
...
async def test_concurrent_error_handling():
    ...
    success_count = sum(
        1
        for r in results
        if not isinstance(r, Exception) and r.status == StreamStatus.COMPLETED
    )
    failure_count = sum(
        1
        for r in results
        if not isinstance(r, Exception) and r.status == StreamStatus.FAILED
    )
    ...
    for i in range(10):  # Try to create more than allowed
        try:
            if i < max_concurrent:
                ...
                raise ValueError(
                    f"Rate limit exceeded: Max {max_concurrent} concurrent streams for {org.plan_type.value} plan"
                )
        except ValueError as e:
