...
class TestHealthMonitor:
    ...
    async def test_register_check(self):
        ...
        await monitor.register_check(
            "test_check", check_func, description="Test check", critical=True
        )
    ...
    async def test_get_overall_health_degraded(self):
        ...
        await monitor.register_check(
            "non_critical_fail", unhealthy_check, critical=False
        )
    ...
    async def test_get_health_report(self):
        ...
        await monitor.register_check(
            "database", healthy_check, "Database check", critical=True
        )
        await monitor.register_check(
            "cache", unhealthy_check, "Cache check", critical=False
        )
