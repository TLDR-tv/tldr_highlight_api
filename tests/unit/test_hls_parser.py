...
class TestStreamQuality:
    ...
    def test_quality_names(self):
        ...
        for resolution, expected_name in test_cases:
            quality = StreamQuality(
                resolution=resolution, bandwidth=1000000, codecs="test"
            )
...
class TestStreamSegment:
    ...
    def test_segment_encryption_detection(self):
        ...
        segment1 = StreamSegment(
            uri="https://example.com/segment001.ts", duration=10.0, sequence_number=1
        )
...
class TestStreamManifest:
    ...
    def test_manifest_playlist_filtering(self):
        ...
        audio_quality = StreamQuality(
            resolution="unknown", bandwidth=128000, codecs="mp4a.40.2", audio_only=True
        )

        video_playlist = HLSPlaylist(
            uri="video.m3u8", quality=video_quality, media_type="video"
        )
        audio_playlist = HLSPlaylist(
            uri="audio.m3u8", quality=audio_quality, media_type="audio"
        )
    ...
    def test_quality_selection(self):
        ...
        qualities = [
            StreamQuality(
                resolution="1920x1080", bandwidth=5000000, codecs="avc1.64001f"
            ),
            StreamQuality(
                resolution="1280x720", bandwidth=2500000, codecs="avc1.42001f"
            ),
...
class TestYouTubeStreamExtractor:
    ...
    def test_is_live_stream_url_available(self):
        ...
        # With HLS URL
        metadata_with_hls = {
            "liveStreamingDetails": {"hlsManifestUrl": "https://example.com/test.m3u8"}
        }
        assert YouTubeStreamExtractor.is_live_stream_url_available(metadata_with_hls)

        # With DASH URL
        metadata_with_dash = {
            "liveStreamingDetails": {"dashManifestUrl": "https://example.com/test.mpd"}
        }
        assert YouTubeStreamExtractor.is_live_stream_url_available(metadata_with_dash)

        # Without URLs
        metadata_without_urls = {
            "liveStreamingDetails": {"actualStartTime": "2023-01-01T12:00:00Z"}
        }
        assert not YouTubeStreamExtractor.is_live_stream_url_available(
            metadata_without_urls
        )
...
class TestQualitySelection:
    ...
    def test_select_optimal_quality_with_bandwidth_limit(self):
        ...
        selected = select_optimal_quality(
            qualities, max_bandwidth=3000000, prefer_quality="best"
        )
...
class TestBandwidthEstimation:
    ...
    def test_estimate_bandwidth_minimum(self):
        ...
        quality = StreamQuality(
            resolution="854x480", bandwidth=1000000, codecs="avc1.42001e"
        )
