    ...
    def test_calculate_velocity_metrics(self, analyzer):
        ...
        for i in range(10):
            analyzer.add_message(
                now + timedelta(seconds=i), f"user{i}", has_emotes=i % 2 == 0
            )
...
class TestSentimentAnalyzer:
    ...
    def analyzer(self):
        return SentimentAnalyzer(
            window_size_seconds=10.0, decay_factor=0.95, min_confidence=0.3
        )
    ...
    def sample_message(self):
        return ChatMessage(
            id="msg1",
            user=ChatUser(id="user1", username="testuser", display_name="TestUser"),
...
class TestIntegration:
    ...
    async def test_full_analysis_workflow(self):
        ...
        for i in range(20):
            messages.append(
                ChatMessage(
                    ...
                    user=ChatUser(
                        id=f"hypeuser{i}",
                        username=f"hypeuser{i}",
                        display_name=f"HypeUser{i}",
                    ),
                    text="HOLY SHIT THAT WAS INSANE!!! PogChamp POGGERS"
                    if i % 2 == 0
                    else "NO WAY! LETS GOOO",
                    timestamp=now + timedelta(seconds=5 + i * 0.2),
                    emotes=[{"name": "PogChamp"}, {"name": "POGGERS"}]
                    if i % 2 == 0
                    else [],
                )
            )
        ...
        velocity = analyzer.temporal_analyzer.calculate_velocity_metrics(
            now + timedelta(seconds=7)
        )

        # Get event impact
        event_impact = analyzer.event_tracker.get_current_impact(
            now + timedelta(seconds=7)
        )
        ...
        assert window.dominant_category in [
            SentimentCategory.HYPE,
            SentimentCategory.EXCITEMENT,
            SentimentCategory.POSITIVE,
            SentimentCategory.VERY_POSITIVE,
        ]  # Allow positive categories too
        ...
        assert (
            confidence > 0.2
        )  # Reasonable confidence for a highlight given spam filtering
