...
class TestFrameSynchronizer:
    ...
    def test_timestamp_normalization(self, synchronizer):
        ...
        ts2 = synchronizer.normalize_timestamp("stream2", 1000000.0)  # 1000 seconds in millis
    ...
    async def test_frame_synchronization(self, synchronizer):
        ...
        # Synchronize frames
        sync_frame1, confidence1 = await synchronizer.synchronize_frame("stream1", frame1)
        sync_frame2, confidence2 = await synchronizer.synchronize_frame("stream2", frame2)
    ...
    async def test_align_frames_with_missing(self, synchronizer):
        ...
        # Register streams
        synchronizer.register_stream("stream1", BufferFormat.HLS_SEGMENT, TimestampFormat.EPOCH_SECONDS)
        synchronizer.register_stream("stream2", BufferFormat.HLS_SEGMENT, TimestampFormat.EPOCH_SECONDS)
    ...
    def test_sync_statistics(self, synchronizer):
        ...
        # Register streams
        synchronizer.register_stream("stream1", BufferFormat.HLS_SEGMENT, TimestampFormat.EPOCH_SECONDS)
        synchronizer.register_stream("stream2", BufferFormat.HLS_SEGMENT, TimestampFormat.EPOCH_MILLIS)
        ...
        assert stats["stream_details"]["stream2"]["timestamp_format"] == TimestampFormat.EPOCH_MILLIS
    ...
    async def test_cleanup(self, synchronizer):
        ...
        assert synchronizer._drift_check_task is None or synchronizer._drift_check_task.done()
        assert synchronizer._sync_update_task is None or synchronizer._sync_update_task.done()
...
class TestSyncIntegration:
    ...
    async def test_multi_stream_synchronization(self):
        ...
        for t in [2.0, 5.0, 8.0]:
            ...
            # Should get frames from all streams
            assert all(aligned[stream] is not None for stream in ["fast_stream", "slow_stream", "drifting_stream"])
            
            # Check timestamps are close
            timestamps = [aligned[stream].timestamp for stream in aligned if aligned[stream]]
            if timestamps:
