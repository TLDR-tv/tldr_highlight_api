...
from src.services.chat_adapters import YouTubeChatAdapter, ChatEventType
from src.services.chat_adapters.youtube import (
    YouTubeMessageType,
)
...
class TestYouTubeChatAdapter:
    ...
    @pytest.mark.asyncio
    async def test_convert_text_message_to_event(
        self, youtube_adapter, sample_youtube_message
    ):
    ...
    @pytest.mark.asyncio
    async def test_convert_super_chat_to_event(
        self, youtube_adapter, sample_super_chat_message
    ):
    ...
    @pytest.mark.asyncio
    async def test_poll_messages_deduplication(
        self, youtube_adapter, mock_session, sample_youtube_message
    ):
    ...
    async def test_adaptive_polling_interval(self, youtube_adapter, mock_session):
        ...
        mock_response = AsyncMock()
        mock_response.status = 200
        mock_response.json.return_value = {"nextPageToken": "token_789", "items": []}
    ...
    async def test_message_cache_cleanup(self, youtube_adapter):
        ...
        assert (
            len(youtube_adapter._seen_message_ids)
            <= youtube_adapter._message_cache_size // 2
        )
