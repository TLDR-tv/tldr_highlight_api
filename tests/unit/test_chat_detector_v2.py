...
class TestEnhancedChatDetector:
    ...
    def sample_messages(self):
        ...
        return [
            OldChatMessage(
                ...
                user_id=f"user{i}",
                username=f"user{i}",
                message="Amazing play! PogChamp" if i % 2 == 0 else "Wow that was sick!",
                platform="twitch",
                metadata={}
            )
            for i in range(10)
    ...
    async def test_detect_features_with_event(self, detector, sample_messages):
        ...
        raid_event = ChatEvent(
            id="raid1",
            type=ChatEventType.RAID,
            timestamp=datetime.fromtimestamp(sample_messages[5].timestamp, tz=timezone.utc),
            data={"viewers": 200}
        )
...
class TestIntegration:
    ...
    async def test_full_detection_workflow(self):
        ...
        # Normal chat
        for i in range(5):
            messages.append(OldChatMessage(
                timestamp=now + i * 2,
                user_id=f"user{i}",
                username=f"user{i}",
        ...
        for i in range(30):
            messages.append(OldChatMessage(
                ...
                message="AMAZING PLAY!!! PogChamp POGGERS" if i % 2 == 0 else "LETS GOOO! That was INSANE!!!",
                platform="twitch"
            ))
        ...
        if results:
            result = results[0]
            assert result.score > 0.25  # Above threshold (adjusted for realistic scoring)
            assert result.confidence > 0.3
            assert result.features["message_count"] == len(messages)
            assert result.features["velocity_score"] > 0  # Should detect velocity increase
            assert result.features["sentiment_score"] > 0.7  # Should have high sentiment
            assert result.metadata["window_sentiment"]["category"] == SentimentCategory.HYPE  # Should detect hype