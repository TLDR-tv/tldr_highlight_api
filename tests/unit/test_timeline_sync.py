...
class TestMultiStreamTimelineSynchronizer:
    ...
    async def test_add_remove_streams(self, multi_synchronizer):
        ...
        stream_sync = await sync.add_stream(
            "stream1", ChatSourceType.TWITCH_EVENTSUB, StreamType.TWITCH_HLS
        )
        ...
        await sync.add_stream(
            "stream2", ChatSourceType.YOUTUBE_LIVE, StreamType.YOUTUBE_HLS
        )
    ...
    async def test_correlation_detection(self, multi_synchronizer):
        ...
        correlating = await sync._find_correlating_highlights(
            "source_stream", source_highlight
        )
...
async def test_integration_scenario():
    ...
    try:
        ...
        for i, text in enumerate(hype_messages):
            user = ChatUser(
                id=f"hype_user{i}", username=f"hype{i}", display_name=f"HypeUser{i}"
            )
            message = ChatMessage(
                id=f"hype_msg{i}",
                user=user,
        ...
        sync.sentiment_analyzer.analyze_window = AsyncMock(
            return_value=window_sentiment
        )
        sync.sentiment_analyzer.temporal_analyzer.calculate_velocity_metrics = Mock(
            return_value=velocity_metrics
        )
        sync.sentiment_analyzer.event_tracker.get_current_impact = Mock(
            return_value=0.8
        )
