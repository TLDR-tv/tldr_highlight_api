...
class TestVideoBufferManager:
    ...
    async def test_remove_buffer(self):
        ...
        # Create buffer
        buffer = manager.get_buffer("stream1")
        assert "stream1" in manager._buffers
...
class TestBufferIntegration:
    ...
    async def test_buffer_with_multiple_formats(self):
        ...
        assert formats == {BufferFormat.HLS_SEGMENT, BufferFormat.FLV_FRAME, BufferFormat.RAW_FRAME}
    ...
    def test_memory_pressure_handling(self):
        ...
        for i in range(100):
            ...
            is_keyframe = (i % 10 == 0)
            frame_size = 50 * 1024 if is_keyframe else 40 * 1024  # 50KB keyframes, 40KB regular
        ...
        keyframe_ratio = sum(1 for f in buffer._frames if f.is_keyframe) / len(buffer._frames)
