...
from src.services.chat_adapters.base import ChatEventType, ChatConnectionError, ChatAuthenticationError
from src.services.chat_adapters.models import MessageType
...
class TestTwitchEventSubAdapter:
    ...
    async def test_create_subscription(self, adapter, mock_session):
        ...
        subscription = await adapter._create_single_subscription("channel.chat.message", "1")
    ...
    async def test_keepalive_timeout(self, adapter):
        ...
        adapter.is_connected = True
        adapter._last_keepalive = datetime.now(timezone.utc) - \
                                  timedelta(seconds=adapter.keepalive_timeout_seconds + 10)
