...
class TestHighlightsRouter:
    ...
    async def test_download_highlight_not_implemented(
        ...
        response = await async_client.get(
            "/api/v1/highlights/test-highlight-id/download"
        )

        assert response.status_code == status.HTTP_501_NOT_IMPLEMENTED
        data = response.json()
        assert (
            data["detail"] == "Highlight download will be implemented in later phases"
        )
    ...
    async def test_delete_highlight_not_implemented(
        ...
        data = response.json()
        assert (
            data["detail"] == "Highlight deletion will be implemented in later phases"
        )
    ...
    async def test_highlight_endpoints_require_db(self, async_client: AsyncClient):
    ...
    async def test_highlight_id_variations(self, async_client: AsyncClient):
        ...
        for highlight_id in highlight_ids:
            ...
            response = await async_client.get(
                f"/api/v1/highlights/{highlight_id}/download"
            )
            assert response.status_code == status.HTTP_501_NOT_IMPLEMENTED
    ...
    async def test_highlight_download_path(self, async_client: AsyncClient):
        ...
        response = await async_client.get(
            "/api/v1/highlights/test-id/download?format=mp4"
        )
        assert response.status_code == status.HTTP_501_NOT_IMPLEMENTED
