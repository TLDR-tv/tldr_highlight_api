...
class TestUserProfileEndpoints:
    ...
    async def test_get_current_user_profile(
        ...
        mock_api_keys = [
            APIKey(id=1, user_id=1, active=True),
            APIKey(id=2, user_id=1, active=True),
        ]
        mock_orgs = [Organization(id=1, owner_id=1)]

        with patch(
            "src.api.routers.users.get_current_user", return_value=mock_current_user
        ):
        ...
        with patch(
            ...
            with patch.object(db_session, "execute") as mock_execute:
                ...
                mock_result.scalar_one_or_none.return_value = User(
                    id=99, email="existing@example.com"
                )
                mock_execute.return_value = mock_result
    ...
    async def test_change_password_success(
        ...
        with patch(
            ...
            response = await async_client.post(
                "/api/v1/users/me/change-password", json=password_data
            )
...
class TestAdminUserEndpoints:
    ...
    async def test_get_user_by_id_admin(
        ...
        with patch(
            "src.api.routers.users.require_admin", return_value=lambda: mock_admin_user
        ):
            with patch.object(db_session, "execute") as mock_execute:
                mock_result = AsyncMock()
...
class TestUserEndpointValidation:
    ...
    async def test_user_permissions_enforcement(
        ...
        response = await async_client.put(
            f"/api/v1/users/{user_id}", json={"email": "new@test.com"}
        )
        assert response.status_code in [
            status.HTTP_401_UNAUTHORIZED,
            status.HTTP_403_FORBIDDEN,
