...
class TestUserAuthentication:
    ...
    async def test_get_current_user_from_api_key_inactive(
        ...
        mock_api_key = APIKey(
            id=1, user_id=123, key=hash_api_key("tldr_sk_test_key"), active=False
        )
    ...
    async def test_get_current_user_invalid_credentials(self, db_session: AsyncSession):
...
class TestOptionalAuthentication:
    ...
    async def test_get_optional_user_authenticated(self, db_session: AsyncSession):
        ...
        with patch(
            "src.api.dependencies.auth.get_current_user", return_value=mock_user
        ):
