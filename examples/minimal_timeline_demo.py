...
class MockFrameSynchronizer:
    ...
    def register_stream(self, stream_id, buffer_format, timestamp_format, is_reference=False):
...
class MockSentimentAnalyzer:
    ...
    async def process_event(self, event):
        ...
        logger.debug(f"Mock: Processed event {event.id} (total: {self.events_processed})")
    ...
    def get_highlight_confidence(self, window_sentiment, velocity_metrics, event_impact=0.0):
        ...
        if hasattr(velocity_metrics, 'spike_detected') and velocity_metrics.spike_detected:
            confidence += 0.3
...
async def demonstrate_timeline_sync():
    ...
    def on_highlight(highlight):
        logger.info(f"🎯 HIGHLIGHT DETECTED!")
    ...
    try:
        ...
        for event in sync_events[:3]:  # Show first 3
            if event.is_synchronized:
                logger.info(f"   📍 Event '{event.event.id}' synced to video time {event.video_timestamp:.2f}s")
