...
async def process_single_stream_example():
    ...
    try:
        ...
        async for segment in adapter.get_segments(duration=10.0, overlap=2.0):
            ...
            logger.info(
                f"  - Average quality: {sum(f.quality_score for f in frames) / len(frames):.2f}"
            )
        ...
        if buffer:
            ...
            logger.info(
                f"  - Buffer duration: {stats['buffer_duration_seconds']:.2f} seconds"
            )
...
async def process_multiple_streams_example():
    ...
    try:
        ...
        def on_frame(stream_id: str, frame):
            logger.debug(
                f"Frame from {stream_id}: timestamp={frame.timestamp:.3f}, keyframe={frame.is_keyframe}"
            )
