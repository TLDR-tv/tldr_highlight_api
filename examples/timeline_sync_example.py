...
async def example_single_stream_sync():
    ...
    def on_highlight_detected(highlight: HighlightCandidate):
        logger.info(
            f"🎯 Highlight detected: {highlight.duration:.1f}s @ {highlight.start_timestamp:.1f}s"
        )
        logger.info(
            f"   Confidence: {highlight.confidence:.2f}, Category: {highlight.category}"
        )
    ...
    try:
        ...
        raid_event = chat_gen.generate_special_event(
            ChatEventType.RAID, viewers=500, from_channel="big_streamer"
        )
        ...
        donation_event = chat_gen.generate_special_event(
            ChatEventType.CHEER, bits=5000, message="TAKE MY MONEY! BEST STREAMER!"
        )
...
async def example_multi_stream_sync():
    ...
    try:
        ...
        for i, highlight in enumerate(multi_highlights, 1):
            ...
            logger.info(
                f"     Duration: {highlight['end_timestamp'] - highlight['start_timestamp']:.1f}s"
            )
        ...
        for stream_id, stream_status in status["stream_status"].items():
            logger.info(
                f"  {stream_id}: {stream_status['buffered_events']} events, "
...
async def simulate_stream_reaction(
    ...
    try:
        ...
        for start_time, end_time, moment_type, activity_rate in vod_moments:
            ...
            for i in range(num_messages):
                ...
                timestamp = vod_start + timedelta(
                    seconds=start_time + duration * progress
                )
            ...
            if moment_type == "climax":
                special_event = chat_gen.generate_special_event(
                    ChatEventType.CHEER, bits=10000, message="TAKE ALL MY BITS!"
                )
        ...
        for i, highlight in enumerate(final_highlights, 1):
            ...
            logger.info(
                f"  {i}. {start_sec:.1f}s - {end_sec:.1f}s "
                f"({highlight.duration:.1f}s) - {highlight.category}"
            )
            logger.info(
                f"     Confidence: {highlight.confidence:.2f}, "
