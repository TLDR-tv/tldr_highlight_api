...
from src.services.chat_adapters.base import ChatEventType, ChatEvent
...
class HighlightDetector:
    ...
    async def process_sync_event(self, sync_event):
        ...
        if sync_event.chat_event and sync_event.chat_event.type == ChatEventType.MESSAGE:
            # Track chat velocity
            ...
            self.chat_velocity_window = [
                e for e in self.chat_velocity_window 
                if e.stream_time > cutoff_time
            ]
            ...
            if len(self.chat_velocity_window) >= self.excitement_threshold:
                ...
                logger.info(f"Potential highlight detected at {sync_event.stream_time:.1f}s: High chat activity")
        ...
        # Check for special events
            ...
            self.potential_highlights.append(highlight)
            logger.info(f"Potential highlight detected at {sync_event.stream_time:.1f}s: {sync_event.chat_event.type.value}")
...
async def main():
    ...
    channel_url = input("Enter Twitch channel URL (e.g., https://twitch.tv/username): ").strip()
    access_token = os.getenv("TWITCH_ACCESS_TOKEN") or input("Enter Twitch access token: ").strip()
    ...
    try:
        ...
        while True:
            ...
            if (datetime.now() - last_status).total_seconds() > 30:
                ...
                logger.info(f"  Highlights detected: {len(detector.potential_highlights)}")
        ...
        if detector.potential_highlights:
            ...
            for i, highlight in enumerate(detector.potential_highlights, 1):
                ...
                if 'sample_messages' in highlight:
                    logger.info(f"   Sample messages: {highlight['sample_messages'][:3]}")
