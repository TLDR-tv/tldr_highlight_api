...
async def process_video_highlight(video_id: str, platform: str) -> Dict[str, Any]:
    ...
    return {"video_id": video_id, "highlights_found": 3, "processing_time": 0.1}
...
async def extract_video_frames(video_url: str) -> int:
    ...
    logfire.info(
        "Starting frame extraction", video_url=video_url, extraction_method="ffmpeg"
    )
    ...
    metrics.record_highlight_processing_time(
        duration_seconds=0.2, stage="frame_extraction", platform="youtube"
    )
...
async def analyze_stream_sentiment(stream_id: str, chat_messages: list) -> float:
    ...
    with with_span(
        "sentiment_analysis", stream_id=stream_id, message_count=len(chat_messages)
    ):
        # Log structured data
        logfire.info(
        ...
        for i, message in enumerate(chat_messages):
            with with_span(
                "analyze_message", message_index=i, message_length=len(message)
            ):
                # Simulate processing
...
class HighlightDetectionService:
    ...
    async def detect_highlights(
        self, stream_id: str, organization_id: str, platform: str
    ) -> Dict[str, Any]:
        ...
        metrics.increment_stream_started(
            platform=platform, organization_id=organization_id, stream_type="live"
        )
        ...
        try:
            ...
            with with_span("chat_analysis_step"):
                chat_sentiment = await analyze_stream_sentiment(
                    stream_id, ["Great play!", "Amazing!", "Wow!"]
                )
            ...
            metrics.increment_stream_completed(
                platform=platform, organization_id=organization_id, success=True
            )
            ...
            # Track failure
            metrics.increment_stream_completed(
                platform=platform, organization_id=organization_id, success=False
            )
    ...
    def _combine_highlights(self, video_highlights: list, sentiment: float) -> list:
        ...
        for vh in video_highlights:
            combined.append(
                {**vh, "combined_score": (vh["confidence"] + sentiment) / 2}
            )
        return combined
...
class StreamRepository:
    ...
    async def get_stream(self, stream_id: str) -> Dict[str, Any]:
        ...
        return {"id": stream_id, "platform": "twitch", "status": "processing"}

    @traced_repository_method()
    async def update_stream_status(self, stream_id: str, status: str) -> None:
        """Update stream status with tracing."""
        logfire.info("Updating stream status", stream_id=stream_id, new_status=status)
    ...
    with with_span("background.cleanup_task", task_type="maintenance"):
        ...
        for i in range(5):
            with with_span(f"cleanup_batch_{i}"):
                ...
                # Track progress
                logfire.info(f"Cleaned up batch {i}", batch_number=i, items_deleted=10)

        # Track task completion
        metrics.increment_task_executed(
            task_name="cleanup_old_highlights", organization_id=None, success=True
        )

        logfire.info("Cleanup task completed", total_deleted=deleted_count)
...
async def main():
    ...
    logfire.configure(
        service_name="tldr-api-example", service_version="1.0.0", console=True
    )
    ...
    try:
        ...
        sentiment = await analyze_stream_sentiment(
            "stream_123", ["Amazing play!", "That was incredible!", "Best stream ever!"]
        )
