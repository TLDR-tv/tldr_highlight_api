...
async def demonstrate_basic_sync():
    ...
    def on_highlight(highlight):
        logger.info(f"🎯 Highlight detected: {highlight.duration:.1f}s, confidence: {highlight.confidence:.2f}")
    ...
    try:
        ...
        logger.info(f"📊 Initial sync status: synchronized={status['is_synchronized']}, "
        ...
        hype_messages = [
            "RAID HYPE!!!", "POGGERS!!!", "WELCOME RAIDERS!!!", 
            "500 VIEWERS!!!", "LETS GOOO!!!", "BEST STREAMER!!!"
        ]
        ...
        for i, highlight in enumerate(all_highlights, 1):
            logger.info(f"   {i}. Duration: {highlight.duration:.1f}s, "
