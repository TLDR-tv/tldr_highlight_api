        ...
        if source_path.exists():
            for py_file in source_path.rglob("*.py"):
                if "__pycache__" not in str(py_file) and not py_file.name.startswith(
                    ...
                    module_name = str(
                        relative_path.parent / relative_path.stem
                    ).replace("/", ".")
                    layer_info["modules"][module_name]["files"].append(
                        str(relative_path)
                    )
...
def generate_report(coverage_data: Dict[str, Dict[str, any]]) -> None:
    ...
    # Overall summary
    overall_coverage = (
        (tested_modules / total_modules * 100) if total_modules > 0 else 0
    )
    ...
    if coverage_data["domain"]["modules"]:
        domain_coverage = (
            sum(
                1 for m in coverage_data["domain"]["modules"].values() if m["has_tests"]
            )
            / len(coverage_data["domain"]["modules"])
            * 100
        )
        if domain_coverage < 50:
            print(
                "1. CRITICAL: Domain layer has minimal test coverage. This should be the top priority."
            )
    ...
    if coverage_data["application"]["modules"]:
        app_coverage = (
            sum(
                1
                for m in coverage_data["application"]["modules"].values()
                if m["has_tests"]
            )
            / len(coverage_data["application"]["modules"])
            * 100
        )
        if app_coverage < 50:
            print(
                "2. CRITICAL: Application use cases are untested. Business logic verification is missing."
            )
